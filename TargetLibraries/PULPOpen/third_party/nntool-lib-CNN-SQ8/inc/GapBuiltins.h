...
#define gap_pack2f16(x, y)              ((v2h)  {(f16)  (x), (f16)  (y)})
#define gap_pack2f16a(x, y)             ((v2ah) {(f16a) (x), (f16a) (y)})
...
#define vfcvt_h_x(__dst, __src) \
{ \
asm("vfcvt.h.x %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define vfcvt_x_h(__dst, __src) \
{ \
asm("vfcvt.x.h %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define vfcvt_ah_x(__dst, __src) \
{ \
asm("vfcvt.ah.x %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define vfcvt_x_ah(__dst, __src) \
{ \
asm("vfcvt.x.ah %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define vfcvt_ah_xu(__dst, __src) \
{ \
asm("vfcvt.ah.xu %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define vfcvt_xu_ah(__dst, __src) \
{ \
asm("vfcvt.xu.ah %0, %1\n": "=r" ((__dst)): "r" ((__src))); \
} while (0)

#define GAP_FLOAT_ROUND_NEAREST_EVEN    0
#define GAP_FLOAT_ROUND_TOWARDS_ZERO    1
#define GAP_FLOAT_ROUND_DOWN            2
#define GAP_FLOAT_ROUND_UP              3
#define GAP_FLOAT_ROUND_NEAREST_MAX     4

#define gap_readfrm(__dst)  \
{                       \
asm(                    \
    "frsr %0\n"         \
    : "=r" ((__dst)));  \
...
#define vfcvt_h_x(__dst, __src) \
{ \
__dst = (v2h) {(f16)(__src)[0], (f16)(__src)[1]}; \
} while (0)

#define vfcvt_x_h(__dst, __src) \
{ \
__dst = (v2s) {(short)(__src)[0], (short)(__src)[1]}; \
} while (0)

#define vfcvt_ah_x(__dst, __src) \
{ \
__dst = (v2ah) {(f16a)(__src)[0], (f16a)(__src)[1]}; \
} while (0)
...
#define vfcvt_xu_h(__dst, __src) \
{ \
__dst = (v2u) {(unsigned short)(__src)[0], (unsigned short)(__src)[1]}; \
} while (0)
